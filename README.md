...
- **캐러셀 슬라이드 내 일원화**: 우측 하단 플로팅 메뉴를 제거하고 캐러셀 첫 번째 슬라이드(`CalendarSlide`) 내부에 입력창(`ItemInput`)과 일정 리스트를 직접 통합.
- **날짜 연동 즉시 입력**: 날짜 네비게이션으로 선택한 날짜에 맞춰 메모 및 할 일을 즉각 추가하고 마감일(Deadline) 지정 지원.
